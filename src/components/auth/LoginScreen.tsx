...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onOpenKYCOnboarding }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4 selection:bg-blue-600 selection:text-white font-sans">
      ...
      <div className="mb-6 text-center animate-in slide-in-from-bottom-4 duration-500">
        <div className="w-16 h-16 mx-auto rounded-3xl bg-blue-700 text-white flex items-center justify-center shadow-xl shadow-blue-700/20 mb-3 border-2 border-blue-600">
      ...
      <div className="w-full max-w-md bg-white border border-blue-100 rounded-3xl p-6 md:p-8 shadow-xl text-slate-900 space-y-6">
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-2 ${
              loginMode === 'tenant' ? 'bg-blue-700 text-white shadow-md shadow-blue-700/20' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-2 ${
              loginMode === 'owner' ? 'bg-blue-700 text-white shadow-md shadow-blue-700/20' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
        ...
        {loginMode === 'tenant' && (
          <form onSubmit={otpStep ? handleVerifyOtp : handleSendOtp} className="space-y-4 animate-in slide-in-from-right-4">
            {!otpStep ? (
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    maxLength={10}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-12 pr-4 py-2.5 text-xs font-mono font-bold text-slate-900 focus:outline-none focus:border-blue-600 shadow-sm"
                    required
                  />
                </div>
              ...
              <div className="animate-in slide-in-from-right-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={() => setOtpStep(false)} className="text-xs text-blue-700 hover:text-blue-900 font-bold">
                    Change number
                  </button>
                </div>
                <input
                  ...
                  placeholder="123456"
                  maxLength={6}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 text-center tracking-widest font-black text-lg focus:outline-none focus:border-blue-600 shadow-sm"
                  required
                />
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-700 hover:bg-blue-800 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-blue-700/20 flex items-center justify-center space-x-2 text-xs disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
            ...
            {onOpenKYCOnboarding && (
              <div className="pt-3 border-t border-slate-100 text-center">
                <button
                  ...
                  className="text-xs text-blue-700 hover:text-blue-900 font-bold flex items-center justify-center space-x-1 mx-auto"
                >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-10 pr-4 py-2.5 text-xs font-semibold text-slate-900 focus:outline-none focus:border-blue-600 shadow-sm"
                  required
                />
              </div>
            </div>
